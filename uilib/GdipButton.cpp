...
HBRUSH CGdipButton::CtlColor(CDC* pScreenDC, UINT nCtlColor) 
{
	if(!m_bHaveBitmaps)
	{
		...
		if (m_dcStd.m_hDC == NULL)
		{
			...
			graphics.DrawImage(*m_pStdImage, 0, 0, rect.Width(), rect.Height());//draw 
		...
		if( (m_dcAlt.m_hDC == NULL) && m_bHaveAltImage )
		{
			...
			graphics.DrawImage(*m_pAltImage, 0, 0, rect.Width(), rect.Height());//ygr  graphics.DrawImage(*m_pAltImage, 0, 0)
			...
			if( (m_dcAltP.m_hDC == NULL) && m_bHaveAltImage )
			{
				...
				graphics.DrawImage(*m_pAltImage, 1, 1, rect.Width(), rect.Height());
